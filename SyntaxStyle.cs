    ...
    public class SyntaxStyle
    {
        ...
        static SyntaxStyle()
        {
            try
            {
                ...
                var regex = new Regex(@"(Keywords\d)|(Operators)\d|(Folder)s( in \w+)|(Delimiters)");
                var styles = xml.XPathSelectElements("//WordsStyle").ToDictionary(
                        ...
                        x => new SyntaxStyle
                        {
                            ...
                            FontWeight = (int.Parse(x.Attribute("fontStyle").Value) & 1) == 1 ? FontWeights.Bold : FontWeights.Normal,
                                        ...
                                        .Where(kw => kw.Groups[0].Success))
                {
                    ...
                    if (name.IsEqual("Delimiters"))
                    {
                        foreach (Match keyword in Regex.Matches(item.Value, @"(00|03|06|09|12|15|18|21)[Cc]"" "))
                        {
                            var index = int.Parse(keyword.Groups[1].Value) / 3 + 1;
                            Tokens[TokenType.String] = styles.At($"Delimiters{index}") ?? Brushes.Black;
                        }
                    }
                    else
                    {
